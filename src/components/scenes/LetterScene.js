...
function LetterScene({ onComplete }) {
  ...
  const paragraphs = [
    ...
    "Anh mong mỗi bước đi của em đều mang theo niềm tin, sự bình an và hạnh phúc. Hãy tiếp tục sống thật trọn vẹn, dám ước mơ, dám yêu thương, và dám sống hết mình — vì thế giới này trở nên tươi đẹp hơn rất nhiều khi có em trong đó.",
    "Cảm ơn em đã luôn là nguồn cảm hứng, là người khiến anh tin rằng tình yêu và nghị lực có thể khiến mọi thứ trở nên tốt đẹp hơn.",
      ...
      <div className="letter-container">
        ...
        <div className="letter-content">
          {paragraphs.map((text, index) => (
            <p
              key={index}
              className={`letter-paragraph ${visibleParagraphs.includes(index) ? 'visible' : ''}`}
            >
              {text}
            </p>
          ))}

          <div className={`letter-signature ${showButton ? 'visible' : ''}`}>
            <p>Yêu em,</p>
